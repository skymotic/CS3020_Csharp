...
namespace OBrien_Final
{
    public class Character
    {
        ...
        public event EventHandler<CharacterEventArgs> ThrownMessage;
        public event EventHandler<CharacterEventArgs> DeathOfCharacter;
        ...
        /// </summary>
        /// <param name="amount"></param>
        protected int DoDamage(int amount)
        {
            if(hp >= amount*10)
            {
                hp -= amount * 10;
        ...
        /// <summary>
        /// Event Activator that triggers character death
        /// </summary>
        public void CharacterDies()
        {
            if (DeathOfCharacter != null)
            {
                DeathOfCharacter(this, new CharacterEventArgs() { CharacterType = characterType });
            }
        }

        /// <summary>
        /// Event Activator that triggers a message to be thrown
        /// </summary>
        /// <param name="str"></param>
        protected void ThrowMessage(string str)
        {
            if(ThrownMessage != null)
            {
                ThrownMessage(this, new CharacterEventArgs() { MessageString = str });
            }
        }
    }
}
